...
CREATE TABLE IF NOT EXISTS movimentacoes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    agente_id UUID NOT NULL REFERENCES agentes_extintores(id) ON DELETE CASCADE,
...
-- Tabela de Checklist de Agentes
CREATE TABLE IF NOT EXISTS checklist_agentes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    agente_id UUID NOT NULL REFERENCES agentes_extintores(id) ON DELETE CASCADE,
    usuario_id UUID NOT NULL REFERENCES auth.users(id),
...
-- Criar índices únicos para evitar conflitos
CREATE INDEX IF NOT EXISTS idx_agentes_tipo ON agentes_extintores(tipo);
CREATE INDEX IF NOT EXISTS idx_agentes_situacao ON agentes_extintores(situacao);
CREATE INDEX IF NOT EXISTS idx_agentes_validade ON agentes_extintores(data_validade);
CREATE INDEX IF NOT EXISTS idx_movimentacoes_agente ON movimentacoes(agente_id);
CREATE INDEX IF NOT EXISTS idx_movimentacoes_usuario ON movimentacoes(usuario_id);
CREATE INDEX IF NOT EXISTS idx_movimentacoes_data ON movimentacoes(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_checklist_agente ON checklist_agentes(agente_id);
CREATE INDEX IF NOT EXISTS idx_checklist_data ON checklist_agentes(data_checklist DESC);
...
CREATE POLICY "Permitir exclusão para usuários autenticados" ON agentes_extintores
...
('LGE', 'Fabricante D', '2024-04-20', 'EM_LINHA', 750.00, 'litros', '2025-04-20'),
('PO_QUIMICO', 'Fabricante E', '2024-05-12', 'ESTOQUE', 25.00, 'kg', '2026-05-12')
